...
async function geteRecord(sessionToken) {
    const summaryData = await getSummary(sessionToken);
    const gameRecord = await readGameRecord(summaryData.url);
    const decryptedGameRecord = decrypt_gameRecord(gameRecord);
...
export default defineEventHandler(async (event) => {
    ...
    const { sessionToken, action } = getQuery(event);
    if (action !== 'encodeSummary' && !sessionToken) {
        return {
            statusCode: 400,
            body: 'Missing sessionToken',
        };
    }
    try {
        let result;
        if (action === 'playerID') {
            ...
            result = await geteRecord(sessionToken)
